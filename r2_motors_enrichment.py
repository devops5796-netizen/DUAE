"""
R2 Motors phone + description enrichment.

Flow:
1) Prepare: read yesterday's DUAE/motors Excel files from R2, read cached users-data.xlsx,
...
3) Combine: merge all job results back into the original Excel sheets, update JSON files,
   and append only successful new users to users-data.xlsx.
"""
...
def yesterday_prefix(date_str: str | None = None) -> tuple[str, str]:
    ...
    prefix = f"{MOTORS_PREFIX}/year={target.year}/month={target.month:02d}/day={target.day:02d}/motors/"
    return target.isoformat(), prefix


def users_key_for_date(target_date) -> str:
    """Build the users-data.xlsx cache key for a given date (rolling cache:
    each day's cache carries forward the previous day's file plus that day's
    newly-found phones)."""
    return (
        f"{MOTORS_PREFIX}/year={target_date.year}/month={target_date.month:02d}/"
        f"day={target_date.day:02d}/motors/users-data/users-data.xlsx"
    )
...
def prepare(date_str: str | None, out_dir: str):
    ...
    users_key_read = users_key_for_date(prev_date)   # rolling cache from the day before
    users_key_write = users_key_for_date(target_date)  # where today's merged cache gets saved
    ...
    for key in keys:
        ...
        for sheet_name, df in sheets.items():
            ...
            for row_pos, (_, row) in enumerate(df.iterrows()):
                ...
                # If phone is already present in the listing, no phone request is needed.
                # If user is cached, no phone request is needed.
            ...
            file_stats[key] = file_stats.get(key, 0) + len(df)

    # Hard guarantee: max 15 listings per job.
...
def combine(results_dir: str, date_str: str | None):
    ...
    # Re-read originals and apply updates while preserving every Excel file and sheet.
